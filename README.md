...
- macOS：下载 `Stream.Hub_0.1.5_*.dmg`。
- Windows：下载 `Stream.Hub_0.1.5_x64-setup.exe` 或 `Stream.Hub_0.1.5_x64_en-US.msi`。

Release 安装包已内置 `libmpv` runtime，普通用户安装后不需要再单独安装 mpv。
...
Windows 本地构建需要确保 `libmpv-2.dll` 可被应用加载；如果要产出自带 runtime 的安装包，需要把 `STREAM_HUB_MPV_RUNTIME_DIR` 指向解压后的 mpv runtime 目录。
